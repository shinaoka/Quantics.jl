...
@testset "fouriertransform.jl" begin
    ...
    @testset "qft" for sign in [1, -1], nbit in [2, 3, 4]
    #@testset "qft" for sign in [1], nbit in [2]
        N = 2^nbit
        ...
        for xin in [0, 1, N-1]
            ...
            mpsf = MPS(sites, collect(string(bitat(xin, pos)) for pos in nbit:-1:1))
            #@show mpsf
            @show MSSTA.qft(mpsf; sign=sign, tag="n")
            mpsg = reduce(*, MSSTA.qft(mpsf; sign=sign, tag="n"))
    
            # Values of output function
            # (k_Q, ...., k_1)
            outfunc = vec(Array(mpsg, reverse(sites)))
