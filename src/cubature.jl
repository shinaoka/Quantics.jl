...
function adaptive_cubature(::Type{T}, D::Int, f, R::Int;
    tolerance::Float64=1e-12, maxbonddim::Int=100, verbosity::Int=0) where {T}
    if D == 1
        @memoize function f__(x)
            return f(x[1])
        end
        return adaptive_quadrature(T, f__, R;
            tolerance=tolerance, maxbonddim=maxbonddim, verbosity=verbosity)
    end
    ...
    return adaptive_quadrature(T, f_, R; tolerance=tolerance, maxbonddim=maxbonddim, verbosity=verbosity)
end
...
"""
function adaptive_quadrature(::Type{T}, f, R::Int; tolerance::Float64=1e-12, maxbonddim::Int=100, verbosity::Int=0)::T where{T}
    ...
    localdims = fill(2, R)
    tree = adaptivetci(T, f_, localdims; maxbonddim=maxbonddim, tolerance=tolerance, verbosity=verbosity, maxnleaves=100000)
